...
**修正ラウンド 3 回目（レビュー単位 A の再レビュー、Important 1 件）**: `JP7251120B2` の見出しが、日英とも請求項 1 の内容と食い違っていた。請求項 1 の「前記第１の車両の加速度が基準以下であることを条件として」は**危険車両の属性ではなく、危険車両を検出して報告する側（第１の車両）に掛かる条件**であるにもかかわらず、旧見出し（ja「低加速の危険車両を検知し周辺車両に位置を警告」/ en "Detecting a low-acceleration risky vehicle and warning nearby vehicles of its location"）はどちらも「低加速の（危険）車両」と誤って読める書き方になっていた（この誤りはもともと日本語側にあり、Task 1.7 で英語を日本語に揃えた際に英語側も同じ誤りを引き継いだ）。請求項 1 を読み直し、条件の掛かり先が第１の車両（観測側）であることが伝わる見出しに書き直した（ja「加速度が基準以下の車の映像から危険車両を検知し周辺へ警告」/ en "Detecting a dangerous vehicle from a low-acceleration vehicle's camera and warning others"）。他の 64 件は変更していない。
...
- 出願年月: 2018-11
- 出願国・地域: JP, US, CN
- 見出し（ja）: 加速度が基準以下の車の映像から危険車両を検知し周辺へ警告
- 見出し（en）: Detecting a dangerous vehicle from a low-acceleration vehicle's camera and warning others
