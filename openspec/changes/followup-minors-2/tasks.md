...
- [ ] 2.1 N1 / D2: RED: `photoIdFromEntry` の unit テスト（`kamo-river-v1.2.yaml` → `kamo-river-v1.2`、`Kamo.yaml` → `Kamo`）と、`kamo-river-v1.2.yaml` を置いた状態でビルドが落ちることの実測。GREEN: `src/lib/photo-meta.ts` に関数を置き、`src/content.config.ts` の photos に `generateId` で渡す。`.` を含む slug の写真ページがビルド・配信できることを確かめ、実測に使った YAML は消す。`tests/e2e/paths.ts` の slug の作り方とコメントを合わせ、未使用の `photoSlugs` の export を消す（F15）
- [ ] 2.2 D3 / H-B1: RED: `validatePhotos([])` が featured の不足を返すテスト。GREEN: 早期 return を消す。`src/lib/content.ts` に `getFeaturedPhoto()` を置き、BaseLayout と index.astro の到達しない throw を消す（H-B3）
