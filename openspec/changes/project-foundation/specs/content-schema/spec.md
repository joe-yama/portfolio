...
`experience`、`certifications`、`achievements` の件数は、日本語版と英語版で一致しなければならない（MUST）。一致しない場合はビルドを失敗させ、どの配列が何件対何件かを示さなければならない（MUST）。

#### Scenario: 件数が一致する
- **WHEN** 日英とも `experience` 3 件、`certifications` 1 件、`achievements` 2 件でビルドする
- **THEN** ビルドは成功する
...
### Requirement: 写真の並び順
写真の一覧を取得するとき、`order` の昇順で返さなければならない（MUST）。

#### Scenario: order 昇順
- **WHEN** `order` が 30、10、20 の写真がある
- **THEN** 一覧は 10、20、30 の順で返る
