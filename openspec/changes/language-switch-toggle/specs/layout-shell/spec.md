...
- **THEN** 「Photos」にはカメラ、「Career」には鞄、言語切り替えのまとまりの先頭には地球儀のドット絵が、トップページ本文の同じ行き先のものと同じ図柄で、支援技術から隠された `<svg>` として 1 つずつ現れる。言語切り替えの `JA` と `EN` の項目の中とロゴには `<svg>` が無い
