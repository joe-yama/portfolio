...
**Goals**: ヘッダーの言語切り替えを `JA / EN` のまとまりにする。トップ本文のサイト内の導線からは言語切り替えを外す（PO 決定 2026-09-25）。

**Non-Goals**: まとまりを共通のコンポーネントに切り出すこと（D3）、トップ本文に別の形で言語切り替えを置くこと、Photos / Career の描き方の変更。
...
- 代案: 相手の言語のリンク 1 つを返す今の形のまま、表示中の言語はページ側で描く → 表示中の言語の項目をページ側で組み立てることになり、並び順（`locales` の順）をデータが保証できない
...
- `.lang-switch` に `border-left: 1px solid var(--line)` と左の余白を付け、Career とのあいだの区切り線にする
...
トップ本文から言語切り替えを外したので（PO 決定 2026-09-25。Photos / Career と同列に並ぶとページへの導線と見分けにくい）、まとまりを描くのは `Header.astro` だけになる。1 か所だけなので共通のコンポーネントには切り出さない。トップ本文の導線は `navLinks()` の 2 つだけを描く。

- 経緯: 当初はトップ本文にも区切り線なしの同じまとまりを置いていた（2026-09-24 実装済み）。PR #63 のレビュー後、PO の判断で外した

### D4: e2e の検査の単位

`links.spec.ts` の検査表で、言語切り替えを `a[hreflang]` ではなく `[role="group"]` で選ぶ。地球儀の svg は、まとまりの直下に 1 つだけあり、項目の中には無いことを確かめる。名前の検査は「English / 日本語」から、グループ名（「言語」 / 「Language」）と各項目の文字（`JA` / `EN`）に置き換える。トップ本文の導線は、直下の要素が Photos・Career の 2 つだけで、`[hreflang]` と `[role="group"]` が無いことを確かめる。`pages.spec.ts:195` の `header a[hreflang="en"]` は、日本語ページでは `EN` のリンクに当たるので、そのまま使える。
...
- [`[role="group"]` の選び方がほかの要素と衝突する] → ヘッダーに、ほかに group は無い。検査はクラスではなく role と名前で選ぶ
