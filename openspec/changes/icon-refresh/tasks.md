...
- [ ] 1.1 `tests/unit/pixel.test.ts`: github / linkedin / briefcase / globe を camera / lost の厳しい番人（16 行 × 16 文字、`^[.#]{16}$`、塗り 1 セル以上）の `describe.each` に統合し、`gridSize` だけの `describe.each` を削除する
...
- [ ] 2.1 `src/lib/pixel.ts` の `briefcase` を描き直す（取っ手を太く、本体をシルエットで判別できる形に。design.md の Decisions）
