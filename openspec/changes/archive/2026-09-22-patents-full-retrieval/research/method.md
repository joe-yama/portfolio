...
実際に取得して分かったこと（design.md には無い実測）: 当初は「英語の見出しの根拠は件ごとに違う」状態だった。design.md D6 が当初定めていた「US → EP → WO の優先順位で代表公報の請求項から起こす」は機械的にはそのとおり実装できたが、**同じ発明でも国ごとに請求項の限定範囲が違う**ため、日英で別の発明に見える行が 6 件出た（`JP7420036B2` `JP7666423B2` `JP7206854B2` `JP7318576B2` `JP7677243B2` `JP7310729B2`）。行に出る `number` と `url` はどちらも同じ JP 公報なので、英語の読者がリンクを開くと見出しと請求項が対応しない事故だった。**PO の指示（2026-09-22, Task 1.7）で、英語の見出しも `number` に採った代表公報（ほぼ JP）の請求項 1 を根拠にするよう統一した**（design.md D12 を書き換え済み）。65 件の内訳は、JP 代表の請求項 1 を根拠にしたもの **63 件**、代表が JP でない同族自身の請求項 1（`/en`）を根拠にしたもの **2 件**（`US20250199754A1`, `US20250146351A1`）。US/EP/WO の請求項を根拠にした件は無くなった。どの公報のどの言語を根拠にしたかは `patents.json` の `titleEnSource` に残しており、全 65 件の内訳は `research/publications.md` の「見出しの根拠言語について」と各ブロックに明記している。
