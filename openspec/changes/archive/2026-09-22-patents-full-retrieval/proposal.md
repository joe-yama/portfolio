...
- `src/content/career/{ja,en}.yaml` の `patents` を、**同族単位**のデータに差し替える
  ...
  - 掲載する名称は、JP 公報の正式名称ではなく請求項 1 から起こした短い見出しにする（下の項目と裁定 D12）
  - **BREAKING ではない**。スキーマも表示も変えず、データだけが変わる
- **掲載する名称を、公報の正式名称から「発明の内容が推測できる短い見出し」に変える**（PO 指示 2026-09-22）。正式名称は「情報処理装置、記録媒体および情報処理方法」のような定型で、65 件の大半が同系統になり読み手が内容を区別できない。見出しは各同族の**請求項 1** から起こし、日本語 40 文字・英語 90 文字を上限としてビルドで検証する。正式名称は `research/publications.md` に残し、PR で PO が全件を照合する
...
- `content-schema`: 「特許のデータ構造」で、(1) `title` の意味を「その言語での発明の名称」から「その言語で、発明の内容が推測できる短い見出し」に変え、長さの上限（日本語 40 文字・英語 90 文字）をビルドで検証する要求を足し、(2) `countries` の先頭が `number` の国・地域と違うときビルドを失敗させる要求を足す（(2) の MUST は既にあるが検証されていない）
- `profile-and-career`: 「特許の表示」で、表示するのを正式名称ではなく短い見出しとし、正式名称を表示してはならないことを定める。あわせて、外部リンクの言語をページの言語に合わせる要求を追加する
