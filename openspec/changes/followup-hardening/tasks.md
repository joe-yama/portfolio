...
- [x] 7.3 `reviewer`（Opus）でブランチ全体を「仕様準拠（5 つの delta）→ コード品質 → ponytail」の順にレビューする。Critical / Important を反映し、Minor は本ファイル末尾の「提案」に転記する。結果を change の GitHub Issue にまとめて 1 回コメントする
  - Important 2 件（teardown が他人の preview を止める経路の残り、経歴ページの条件分岐に番人が無い）を修正し、再レビューで Approved。Issue #29 にコメント済み
- [x] 7.4 `gh api user --jq .login` が `joe-yama` であることを確認してから push し、`Closes #<Issue 番号>` を本文に含む PR を作る。CI が緑であることを確認して Issue にコメントする
  - PR #35。CI の job `check` は pass（1m51s）、Deploy は PR では動かないことを `gh run list` で確認（`push: main` のみ）
