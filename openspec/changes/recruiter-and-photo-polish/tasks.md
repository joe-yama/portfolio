...
- 2560×1440 のような非常に広い画面では、トップの右の列の文字（幅約 260px）が列の左に寄り、その右に約 750px の空白が出る（1920×1080 では約 480px）。spec には反しない。PO 判断
- 64rem 以上で高さの上限が効く画面（例 1366×650）では、写真を列の左に寄せたので、写真と文字の間が列の間 2rem より広がる（1366×650 で約 78px）。spec には反しない。PO 判断
