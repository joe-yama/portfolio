...
- 作成日: 2026-09-17
- 状態: brainstorming で PO 承認済み（5 セクションすべて）。次は writing-plans で実装計画を作る
...
| 更新方法 | リポジトリのファイル（YAML / 画像）を編集して push。管理画面なし |
...
ブログ・近況の一覧、問い合わせフォーム（メールリンクで代替）、ライトボックスや無限スクロールなどの JavaScript 演出、検索、コメント、アクセス解析、外部 CDN や外部フォント配信、PR ごとのプレビュー環境、視覚回帰テスト、Lighthouse のスコア閾値。
...
└── photos/    <slug>.yaml        # 写真 1 枚 = 1 ファイル。二言語を同居
src/assets/photos/<slug>.jpg      # 写真の元画像
...
```yaml
image: ../../assets/photos/2025-kyoto-dawn.jpg   # image() ヘルパーで存在と形式を検証
...
- スキーマで表現できない制約（`featured` がちょうど 1 枚、`order` の重複なし、slug がファイル名と一致）は独自の検証関数で行い、ビルド前に失敗させる
...
- 写真は JPEG、長辺 2,500px 程度、sRGB、品質 90 前後で入稿。RAW や高解像度の元データはリポジトリに入れない
- 合計サイズは 100 MB 以下を目安にする（数十枚 × 1〜2 MB）
- `npm run photo:add <画像ファイル>` で EXIF を読み取り、`src/content/photos/<slug>.yaml` の雛形（image、takenAt、exif の 5 項目）を生成する。title / location / alt は手で書く。EXIF 読み取りは開発時だけで、公開サイトには影響しない（依存: exifr）
...
- `<Picture>` コンポーネントを使い、`formats={['avif','webp']}` と `widths` を指定する
...
| 層 | ツール | 確認すること | タイミング |
|---|---|---|---|
| 内容の検証 | Zod スキーマ + 独自検証 | 必須項目、型、画像の存在、`featured` が 1 枚、`order` 重複なし | ビルド時 |
| 単体テスト | Vitest | 撮影情報の整形、言語切り替え URL、日英の件数一致、並び順、EXIF 雛形生成 | Stop hook と CI |
...
- レビュー用サブエージェントは `npm run build` → `npm run preview` → Playwright MCP で実操作する（`file:` URL は使えない）

### 8.1 ハーネスへの反映（同じ change で行う）

1. `.claude/rules/testing.md` にコマンドを記載: `npm test`（Vitest）、`npm run lint`（Biome）、`npm run typecheck`（astro check）、`npm run e2e`（Playwright）
2. `.claude/hooks/lint-on-edit.sh` の `detect_lint()` を `npx biome check <file>` に、`.claude/hooks/test-on-stop.sh` の `detect_test()` を `npm test` に置き換える
3. `.claude/settings.json` の `permissions.allow` に `npm run lint / typecheck / e2e / build / preview` と `npx biome` を追加
...
| 流れ | 内容 |
|---|---|
| Pull Request | `astro check` → Biome → Vitest → ビルド → Playwright e2e。すべて通らないとマージ不可（`main` のブランチ保護は PO が GitHub 上で設定） |
| `main` へマージ | `withastro/action@v6` でビルド、`actions/deploy-pages@v5` で GitHub Pages へ公開 |
| 独自ドメイン | `public/CNAME` にドメイン名、`astro.config` の `site` に `https://<ドメイン>`、`base` は設定しない。DNS 登録は PO の作業。HTTPS は GitHub Pages が自動発行 |
| Node | `.node-version` で固定（CI と同じ）。パッケージマネージャは npm |
...
2. **写真**: 初回に載せる数十枚（入稿ルールは 5.3）と、代表写真 1 枚の指定
