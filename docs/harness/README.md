...
- **プラグインのプロジェクトスコープの導入記録**（2026-09-25 実測）: `claude plugin install --scope project` の記録（`~/.claude/plugins/installed_plugins.json`）は実行したディレクトリのパスに付く。portfolio は移行時の worktree（削除済み）のパスで記録されているが、main と他の worktree でも `.claude/settings.json` の `enabledPlugins` からキャッシュの最新版（0.2.1）が読まれ、guard は効いている。marketplace の固定（`extraKnownMarketplaces` の `ref`）は、そのリポジトリでセッションを起動したときにユーザー側の登録へ反映され、その後の `claude plugin update harness@agentic-harness --scope project` で版が上がる
