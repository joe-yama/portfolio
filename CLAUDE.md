...
ハーネス構築完了（2026-09-17）、設計書 `docs/superpowers/specs/2026-09-17-portfolio-site-design.md` を PO 承認・レビュー反映済み（2026-09-17、Change 2 の決定を 2026-09-18 に反映、公開先の決定を 2026-09-21 に反映）。`docs/HANDOFF.md` §3 のセットアップ手順は再実行しない。**Change 1〜9 はすべてマージ・アーカイブ済み**（`openspec/changes/archive/`、main spec は `openspec/specs/{content-schema,deployment,i18n-routing,layout-shell,photo-pipeline,profile-and-career,quality-gates,sitemap}` の 8 つ）:
...
- Change 9 `photo-height-cap`（Issue #23）= PR #28（2026-09-21。トップの代表写真と写真の個別ページの写真に表示高さの上限を CSS だけで設け、名前・肩書・連絡先・導線が第一画面に収まるようにした。`PhotoPicture` の `img.full` に `max-height: var(--photo-max-height, none)` を置き、値はページ側が `--photo-max-height` で渡す（トップ `max(12rem, 100svh - 27rem)`、個別ページ `max(12rem, 100svh - 18rem)`）。`tests/e2e/viewport.spec.ts` を新設し e2e は 27 → 45 件。main spec `photo-pipeline` / `profile-and-career` に要求を追加し `quality-gates` の「ビルド出力のブラウザ検査」を更新。**レビュー結果と裁定は Issue #23 のコメント**、後続への提案は `openspec/changes/archive/2026-09-21-photo-height-cap/tasks.md` の末尾）
...
Change 9 では **「テストが緑」と「テストが番人」は別物**という形で 2 回出た。どちらも CSS の寸法検査で、書いた本人もレビュー前の実行も気づけなかった: (1) `grid` 変種にも高さ上限を漏らす変異が 1440×900 のサムネイル検査を落とさなかった（`sizes` の推定幅 20rem=320px が実際の列幅 約294px を上回り `max-width: 100%` で丸め込まれる。800×600 = `sizes` が `33vw` になる帯を足して塞いだ）、(2) `max(12rem, …)` の下限を外す変異がどの検査も落とさなかった（e2e が使う 4 つの viewport では式が常に正。1280×400 の検査を足して塞いだ）。**番人を書いたら必ず変異を当てて落ちることを確かめる**（change の `tasks.md` に「番人が本当に番人か確かめる」タスクを 1 つ置く）。隔離実行の手順は `docs/harness/README.md`。
