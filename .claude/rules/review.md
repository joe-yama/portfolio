...
- 実装したコンテキストはレビューしない。レビューは必ず、実装とは別のコンテキストを持つサブエージェントが行う。
...
- Agent ツールでは常に `subagent_type` を `implementer`（Sonnet）か `reviewer`（Opus）にする。**モデルは agent 定義に任せて省略しない**（省略するとセッションのモデル = 最上位・最高額を継承する）。
